...
void read_chat(void *thread_data)
{
	ssize_t n;
	unsigned char buf[DATA_SIZE];

    	int sd = ((struct thread_struct*)thread_data)->sd;
    	int cfd = ((struct thread_struct*)thread_data)->crypto_fd;
...

	{
		...
		//if (insist_write(0, data.decrypted, n) != n)
		//{
		//	perror("write");
		//	exit(1);
		//}
...
void write_chat(void *thread_data)
{
	size_t n;
...
int main(int argc, char *argv[])
{
	...
	thread_data.sd = sd;
	thread_data.crypto_fd = crypto_fd;
	...
	//rc = pthread_create(&read_thread, NULL, read_chat, &thread_data);
	//if (rc)
	/*{
          printf("ERROR; return code from 'read' pthread_create() is %d\n", rc);
          exit(-1);
    }

	rc = pthread_create(&write_thread, NULL, write_chat, &thread_data);
	if (rc)
	{
          printf("ERROR; return code from 'write' pthread_create() is %d\n", rc);
          exit(-1);
    }
